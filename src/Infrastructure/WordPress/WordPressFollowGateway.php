...
final class WordPressFollowGateway implements FollowGateway {
	...
	private function ids( $value, int $owner_id = 0 ): array {
		$ids = is_array( $value ) ? array_values( array_unique( array_filter( array_map( 'absint', $value ) ) ) ) : array();
		return $owner_id > 0 ? array_values( array_diff( $ids, array( $owner_id ) ) ) : $ids;
	}

	public function followers( int $user_id ): array {
		return $this->ids( get_user_meta( $user_id, 'followers', true ), $user_id );
	}

	public function following( int $user_id ): array {
		return $this->ids( get_user_meta( $user_id, 'following', true ), $user_id );
	}

	public function persist_followers( int $user_id, array $followers ): bool {
		return false !== update_user_meta( $user_id, 'followers', $this->ids( $followers, $user_id ) );
	}

	public function persist_following( int $user_id, array $following ): bool {
		return false !== update_user_meta( $user_id, 'following', $this->ids( $following, $user_id ) );
	}

	public function with_lock( int $actor_id, int $target_id, callable $callback ) {
		$ids = array( $actor_id, $target_id );
		sort( $ids, SORT_NUMERIC );
		$key      = '_metafans_follow_lock_' . md5( implode( ':', $ids ) );
